...
function NotifyManager:requireNofifyContentById(notifyId)
	...
	if self:checkIsValidNotify(notify) then
		local gameNoticeUrl = LoginWorld.Instance:getServerMgr():getNoticeUrl()	
		local myGameNoticeUrl = string.gsub(gameNoticeUrl, "{id}", notifyId)					
		HttpTools:getInstance():send(myGameNoticeUrl, kTypePost, eHttpReqTag.NotifyContent, 0, 0)													
	end
end
...
function NotifyManager:handleNotifyContents(state, responeData)
	if state == 200 then
		...
		if data and type(data)=="table" then				
			...
			if notify then
				if data.noticeTitle then
					local title = string.gsub(data.noticeTitle, "+", " ")
					notify:setNotifyTitle(self:unescape(title))
				end
				
				if data.noticeName then
					local noticeName = string.gsub(data.noticeName, "+", " ")
					notify:setNotifyName(self:unescape(noticeName))
				end
				...
				if data.content then
					...
					notify:setNotifyContent(self:unescape(content))
				end
...
function NotifyManager:getMaintainNotifyMsg(notify)		
	...
	local msg = Config.LoginWords[15002] .. notify:getNotifyTitle() .. "\n"					
	msg = msg .. Config.LoginWords[15003] .. notify:getNotifyName() .. "\n"
	msg = msg .. Config.LoginWords[15004].."\n" .. notify:getNotifyContent()
	return msg										
end
--维护公告专用
function NotifyManager:showMaintainNotifyView(notifyData)
	local msg = self:getMaintainNotifyMsg(notifyData)
	if not msg then	
		return
	end
	...
	msgBox:setNotify(notify, notifyData)
	msgBox:layout()		
