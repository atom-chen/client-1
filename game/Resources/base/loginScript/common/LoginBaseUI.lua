...
LoginBaseUI = LoginBaseUI or BaseClass()
...
function LoginBaseUI:touchHandler(eventType, x, y)
	if self.rootNode and self.rootNode:isVisible() and self.rootNode:getParent() then	
		...
		if rect.containsPoint and rect:containsPoint(point) then
			return 1
		else
			return 0
		end
...
-- 等同于CCNode的addChild
function LoginBaseUI:addChild(node, zorder, tag)
	if node then
...
function LoginBaseUI:removeChild(node)
	if (node) then
...
function LoginBaseUI:getRootNode()
	return self.rootNode
end

function LoginBaseUI:getContentNode()
	return self.contentNode
end

function LoginBaseUI:SetSize(size)
	self.rootNode:setContentSize(size)
	local rootNodeSize = CCSizeMake(size.width-42, size.height-69)
	self.contentNode:setContentSize(rootNodeSize)	
...
-- 把窗口居中
function LoginBaseUI:makeMeCenter()
...
-- 设置左上角图标
function LoginBaseUI:setFormImage(imageNode, offset)
	if imageNode then
		...
		--[[if not  self.imageBg then
			self.imageBg = createSpriteWithFrameName(RES("login_common_lefttop_bg.png"))
...
-- 设置标题
function LoginBaseUI:setFormTitle(titleNode, align)
	if titleNode then
...
function LoginBaseUI:initFullScreen()
	local fullScreenSize = CCSizeMake(874, 564)
	self:init(fullScreenSize)
	return fullScreenSize
end

function LoginBaseUI:initHalfScreen()
...
function LoginBaseUI:init(size, pos)
	...
	if self.enableReload and os.getenv("OS") == "Windows_NT" then
		local btnReload = createButtonWithFramename(RES("login_btn_close.png"), RES("login_btn_close.png"))
...
function LoginBaseUI:updateContentNode()
...
function LoginBaseUI:initWithBg(size, bgImage, bShowCloseBtn, bShowTitleBg, rrect)
...
function LoginBaseUI:removeFromRootNode(node)
	if (node) then
...
function LoginBaseUI:reload()
	if self.packageName  then
		package.loaded[self.packageName] = nil
		require(self.packageName)
--		UIManager.Instance:removeUI(self.viewName)
		UIManager.Instance:showUI(self.viewName)
	end
end
--delay延迟多少秒消失，默认3秒 touchArea:可点击区域默认是屏蔽整个rootNode的点击事件的
function LoginBaseUI:showLoadingHUD(delay,func,touchArea)
	local manager =UIManager.Instance
	manager:showLoadingHUD(delay,self.rootNode,func,touchArea)
end
--关闭loadingHUD
function LoginBaseUI:hideLoadingHUD()
	local manager =UIManager.Instance
	manager:hideLoadingHUD()
end

function LoginBaseUI:addTopLeftNode(node,title,pos)
	if not pos then
		pos = ccp(0,0)
	end
	if not self.topLeftNode then
		self.topLeftNode = CCNode:create()
		self.rootNode:addChild(self.topLeftNode, UIZOrder.TopLeftNode)
		VisibleRect:relativePosition(self.topLeftNode,self.rootNode,LAYOUT_TOP_INSIDE+LAYOUT_LEFT_INSIDE,pos)
	else
		self.topLeftNode:removeAllChildrenWithCleanup(true)
		self.topLeftNode:setPosition(ccp(0,0))
		VisibleRect:relativePosition(self.topLeftNode,self.rootNode,LAYOUT_TOP_INSIDE+LAYOUT_LEFT_INSIDE,pos)
	end
	if node then
		local nodeSize = node:getContentSize()
		self.topLeftNode:addChild(node)
		VisibleRect:relativePosition(node,self.topLeftNode,LAYOUT_LEFT_INSIDE,ccp(-5,-nodeSize.height/2-15))
		if title then
			self.topLeftNode:addChild(title)
			VisibleRect:relativePosition(title,node,LAYOUT_CENTER+LAYOUT_RIGHT_OUTSIDE,ccp(scale*9,0))
			VisibleRect:relativePosition(title,self.topLeftNode,LAYOUT_TOP_INSIDE,ccp(0,-scale*43))
		end
	end
end

function LoginBaseUI:createCloseBtn(layoutPoint)
	self:removeFromRootNode(self.btnClose)
	
	self.btnClose = createButtonWithFramename(RES("login_btn_close.png"), RES("login_btn_close.png"))
...
--隐藏关闭按钮
function LoginBaseUI:setVisiableCloseBtn(flag)
...
--重写该函数
function LoginBaseUI:getNodeByName(name)
	return nil
end

function LoginBaseUI:createBackground(size, bgImage, rect)
	self:removeFromRootNode(self.background)
	if (bgImage == nil) then
		bgImage = RES("login_squares_bg1.png")
	end		
	if (rect) then
...
--关闭按钮点击时的回调函数
function LoginBaseUI:onCloseBtnClick()
	return true
end

function LoginBaseUI:createVipFrame(size, title)
	...
	-- 背景图
	local tmptopBg = createScale9SpriteWithFrameNameAndSize(RES("login_vip_fram2.png"), CCSizeMake(frameSize.width-70, 46))
	local topMidLeftFrame = createSpriteWithFrameName(RES("login_vip_fram3.png"))
	local topMidRightFrame = createSpriteWithFrameName(RES("login_vip_fram3.png"))
	topMidRightFrame:setFlipX(true)
	local topLeftFrame = createSpriteWithFrameName(RES("login_vip_fram1.png"))
	local topRightFrame = createSpriteWithFrameName(RES("login_vip_fram1.png"))
	topRightFrame:setFlipX(true)
	
	
	--下方
	local bottomScaleFrame = createScale9SpriteWithFrameNameAndSize(RES("login_vip_fram4.png"), CCSizeMake(frameSize.width-23*2, frameSize.height-40))
	local bottomLeftFrame = createSpriteWithFrameName(RES("login_vip_fram5.png"))
	local bottomRightFrame = createSpriteWithFrameName(RES("login_vip_fram5.png"))
	...
	VisibleRect:relativePosition(topMidLeftFrame, self.rootNode, LAYOUT_TOP_INSIDE + LAYOUT_CENTER, ccp(-topMidLeftFrame:getContentSize().width/2 + 1, 0))
...
function LoginBaseUI:createVipFrameCloseBtn(btntype,func,pos)
	if btntype==ClosebtnType.Back then
		self.btnVipCloseBtn = createButtonWithFramename(RES("login_btn_back.png"))
	else
		self.btnVipCloseBtn = createButtonWithFramename(RES("login_btn_close.png"))
	end
