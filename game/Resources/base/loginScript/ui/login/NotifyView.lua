...
NotifyView = NotifyView or BaseClass(LoginBaseUI)
...
function NotifyView:initBg()
	--local titleNode = createLabelWithStringFontSizeColorAndDimension(Config.LoginWords[15010], "Arial", FSIZE("Size4"), FCOLOR("ColorYellow1"))	
	...
	local rightsecondBg = createScale9SpriteWithFrameNameAndSize(RES("login_squares_bg2.png"), CCSizeMake(813, 479))
	...
	local leftSecondBg = createScale9SpriteWithFrameNameAndSize(RES("login_squares_bg3.png"), CCSizeMake(180, 465))
	...
	self.scrollViewBg = createScale9SpriteWithFrameNameAndSize(RES("login_squares_bg3.png"), CCSizeMake(612, 465))
	...
	self.titleBg = createScale9SpriteWithFrameNameAndSize(RES("login_suqares_goldFrameBg.png"), CCSizeMake(612, 29))	
...
function NotifyView:createTabView()
	...
	local createBtn = function (key, notify)
		local button = createButtonWithFramename(RES("login_rank_nomal_btn.png"), RES("login_rank_select_btn.png"))
	...
	--设定最上面的为默认选中
	if self.tagView.setSelControl and btnArray:lastObject() then
		self.tagView:setSelControl(btnArray:lastObject())
	end
