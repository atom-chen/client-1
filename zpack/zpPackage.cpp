...
bool Package::readFilenames()
{
	...
	{
		//not compressed
		size_t i= fread(&dstBuffer[0], m_header.allFilenameSize, 1, m_stream);
	}
	else
	{
		vector<u8> tempBuffer(m_header.allFilenameSize);
		size_t i= fread(&tempBuffer[0], m_header.allFilenameSize, 1, m_stream);
...
void Package::removeDeletedEntries()
{
	...
	for (u32 i = 0; i < fileCount;)
	{
		...
		if ((entry.flag & FILE_DELETE) != 0)
		{
			m_fileEntries.erase(m_fileEntries.begin() + i * m_header.fileEntrySize, m_fileEntries.begin() + (i+1) * m_header.fileEntrySize);
...
bool Package::buildHashTable()
{
	...
	for (u32 i = 0; i < fileCount; ++i)
	{
		...
		if((currentEntry.flag & FILE_DELETE) == 0)
			m_hashTable[index] = i;
	}
	return !wrong;
}
...
bool Package::insertFileHash(u64 nameHash, u32 entryIndex)
{
	...
	while (m_hashTable[index] != -1)
	{
		...
		else if ((entry.flag & FILE_DELETE) != 0 && entry.nameHash == nameHash)
			break;
		if (++index >= tableSize)
		{
			index = 0;
...
void Package::setMainVersion( u8 version)
{
	m_header.mainVersion = version;
	m_dirty = true;
}

zp::u8 Package::getMainVersion() const
{
	return m_header.mainVersion;
}

void Package::setSubVersion( u32 version )
{
	m_header.subVersion = version;
	m_dirty = true;
}

zp::u32 Package::getSubVersion() const
{
	return m_header.subVersion;
}

void Package::setOldSubVersion( u32 version )
{
	m_header.oldSubVersion = version;
	m_dirty = true;
}

zp::u32 Package::getOldSubVersion() const
{
	return m_header.oldSubVersion;
}
