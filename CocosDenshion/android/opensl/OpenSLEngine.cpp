...
extern "C" {
	...
	static jclass getClassID(JNIEnv *pEnv)
	{
		...
		if (! ret)
		{
			LOGD("%s", "Failed to find class of %s", CLASS_NAME);
		}
	...
	static bool getStaticMethodInfo(JniMethodInfo &methodinfo, const char *methodName, const char *paramCode)
	{
		...
		do 
		{
			...
			if (! methodID)
			{
				LOGD("%s", "Failed to find static method id of %s", methodName);
				break;
			}
...
SLInterfaceID getInterfaceID(const char *value)
{
	...
	if (errorInfo)
	{
		LOGD("%s", "Get interface id: %s from OpenSL failed", errorInfo);
		IID = NULL;
	}
	return *IID;
}
...
void* getFuncPtr(const char *value)
{
	...
	if (errorInfo)
	{
		LOGD("%s", "Get function from OpenSL failed: %s", errorInfo);
		funcPtr = NULL;
	}
	return funcPtr;
}
...
bool initAudioPlayer(AudioPlayer* player, const char* filename) 
{
	...
	if (FILE_NOT_FOUND == fd)
	{
		...
		LOGD("%s", "file not found! Stop preload file: %s", filename);
		return false;
	}
